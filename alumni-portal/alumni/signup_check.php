<?php
	...
	if (empty($email)) {
		...
		$stud_id_checker = $connection->query("SELECT * FROM alumni WHERE studNo ='".$studno."'");
		...
		if (mysqli_num_rows($result) > 0) {
           ...
           $sql2 = "INSERT INTO alumni(id, firstname, middleName, lastname, photo, gender, birthday, email, contact, socMed, interest, schoolAttended, CurrentWork, courseGraduated, created_on, password, address, studNo) VALUES(null,'$firstname','$middlename','$lastname', ' ', '$gender','$birthday', '$email', '$contact', '-', ' ', ' ', ' ', ' ','now()','$pass','$address','$studno')";
