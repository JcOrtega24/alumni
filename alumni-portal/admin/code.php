...
if (isset($_POST['employeeInsert'])) {
    ...
    if ($password === $confirmpassword) {
        $emp_id_checker = $connection->query("SELECT * FROM accounts where userData ='Employee' AND IdNumber='" . $employeeNo . "'");
        $result_emp_id_checker = $emp_id_checker->num_rows;
        if ($result_emp_id_checker == 0) {
            $emp_email_checker = $connection->query("SELECT * FROM accounts where userData ='Employee' AND email='" . $email . "'");
            $result_emp_email_checker = $emp_email_checker->num_rows;
            if ($result_emp_email_checker == 0) {
                $pass = password_hash($password, PASSWORD_DEFAULT);
                $query = "INSERT INTO accounts(IdNumber, Username, Password, FirstName, LastName, MiddleName, Email, ContactNumber, BranchId, DepartmentId, AddedDate, userData) 
                    VALUES(
                        '$employeeNo',
                        '$username',
                        '$pass',
...
if (isset($_POST['deleteEmployee'])) {
    ...
    $queryadmin = "SELECT * FROM accounts where id='$id'";
...
if (isset($_POST['employeeInsert1'])) {
    ...
    $confirmpassword = $_POST['confirmpassword'];
    if ($password === $confirmpassword) {
        $admin_email_checker = $connection->query("SELECT * FROM accounts WHERE userData ='Administrator' AND email ='" . $email . "'");
...
if (isset($_POST['deleteAlumni'])) {
    ...
    if ($query_run) {
        ...
        if ($query_run_logs) {
            ...
            //deleting of alumnisurvey
            $query_del_studforms = "DELETE FROM alumnisurvey where studID='$id'";
...
if (isset($_POST['alumniInsert'])) {
    ...
    $elementaryGraduated = $_POST['year_elementary'];
    $elementaryHonors = $_POST['honor_elementary'];
    $highschool = $_POST['highschool'];
    $highschoolGraduated = $_POST['year_highschool'];
    $highschoolHonors = $_POST['honor_highschool'];
    $college = $_POST['college'];
    $collegeDegree = $_POST['degree'];
    $collegeGraduated = $_POST['year_college'];
    $collegeHonors = $_POST['honor_college'];
    if ($random) {
        $pass = password_hash($password, PASSWORD_DEFAULT);
        // $pass = password_hash($random, PASSWORD_DEFAULT);

        $alumni_id_checker = $connection->query("SELECT * FROM alumni where email ='" . $email . "'");
        $row_cnt_email = $alumni_id_checker->num_rows;
        if ($row_cnt_email == 0) {
            $survey = $connection->query("SELECT * FROM alumni WHERE studNo ='" . $studNo . "'");
            ...
            if ($row_cnt == 0) {
                ...
                $query = "INSERT INTO alumni
                (id, firstname, middleName, lastname, photo, gender, birthday, email, contact, interest, schoolAttended, courseGraduated, created_on, password, companyAddress,mailingAddress, studNo,studRemarks, fbURL, twitterURL, elementary, elementaryGraduated, elementaryHonors, highschool, highschoolGraduated, highschoolHonors, college, collegeDegree, collegeGraduated, collegeHonors)
                VALUES($raw_id, '$firstname','$middlename','$lastname', ' ', '$gender','$birthday', '$email', '$contact', '$testSchoolID', '$schoolAttended', '$course',NOW(),'$pass','$companyAddress', '$mailingAddress','$studNo','$studRemarks', '$facebook', '$twitter', '$elementary', '$elementaryGraduated', '$elementaryHonors', '$highschool', '$highschoolGraduated', '$highschoolHonors', '$college', '$collegeDegree', '$collegeGraduated', '$collegeHonors')";
                // die($query);
                $query_run = mysqli_query($connection, $query);
                if ($query_run) {
...
if (isset($_POST['alumniEdit'])) {
    ...
    if ($password === $confirmpassword) {
        if (isset($_POST['yeargraduated']) || isset($_POST['course'])) {
            ...
            elementaryGraduated = '$elementaryGraduated',
...
if (isset($_POST['delete_row'])) {
    ...
    foreach ($checkArray as $id) {
        $query_mul_delete = "DELETE FROM alumni WHERE id ='$id'";
        ...
        if ($query_run_mul_delete) {
            ...
            if ($query_run_logs) {
                ...
                $query_del_alumnisurveys = "DELETE FROM alumnisurvey where studID ='$id'";
                $query_run_del_alumnisurveys = mysqli_query($connection, $query);
            }
        }
    }
